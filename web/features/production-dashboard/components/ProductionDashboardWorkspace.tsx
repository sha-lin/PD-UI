...
export default function ProductionDashboardWorkspace(): ReactElement {
    const overviewQuery = useQuery({
        queryKey: ["production-dashboard", "overview"],
        queryFn: (): Promise<DashboardOverview> => fetchDashboardOverview(),
    });

    const analyticsQuery = useQuery({
        queryKey: ["production-dashboard", "analytics"],
        ...
        <>
            <header className="bg-white shadow">
                <div className="px-8 py-4">
                    <h1 className="text-2xl font-bold text-brand-black">Production Dashboard</h1>
                    <p className="mt-1 text-sm text-gray-600">
                        Track production flow, monitor alerts, and act on work queues.
                    </p>
                </div>
            </header>
